...
class FastCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        for module in self.fast_cnn:
            x_m = module(x).squeeze()
            # print(x_m.shape)
            # if x_m.shape == torch.Size([32]):
            #     x_m = x_m.expand(2,-1)
            x_out.append(x_m)
        # print(x_out)
...
class EncodingPart(nn.Module):
    ...
    def __init__(
        ...
        self.shared_image = nn.Sequential(
            nn.Linear(2048, 256),  # originally 512
            nn.BatchNorm1d(256),
            nn.ReLU(),
            nn.Dropout(),
...
class SimilarityPart(nn.Module):
    def __init__(
            self,
            shared_dim=128,
            sim_dim=64):
        super(SimilarityPart, self).__init__()
        self.prepare_mlp = nn.Linear(200, 768)
    ...
    def forward(self, text, image):
        ...
        # similarity_specific layer
        text_aligned = self.text_aligner(text_encoding)  # projection head
        image_aligned = self.image_aligner(image_encoding)  # projection head
...
class UnimodalDetection(nn.Module):
    def __init__(self, shared_dim=128, prime_dim=16, pre_dim=2):
...
class CrossModule4Batch(nn.Module):
    ...
    def forward(self, text, image):
        # batch数据作为输入
        text_in = text.unsqueeze(2)  # B * text_dim * 1
        image_in = image.unsqueeze(1)  # B * 1 * image_dim
        corre_dim = text.shape[1]
        similarity = torch.matmul(text_in, image_in) / \
                     math.sqrt(corre_dim)  # corre_dim corre_dim
        # B * corre_dim *  corre_dim -> 1
